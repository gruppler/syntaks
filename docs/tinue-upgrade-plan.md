...
- **Defender nodes** (`search_defender`): when restricted, generate only moves that address the attacker's road threat (block the threatened square: flat/wall/cap placement there, plus stack moves) or make the defender's own road. This is the pruning that was reverted as *unsound for the full search* — it is **sound only in restricted mode**, where the attacker is guaranteed to have a live tak. Gate it strictly behind `TakChain`.
...
**Strict, always-safe fast-outs** (cheap, apply everywhere): the position already
has a road (not a candidate — already handled); or the attacker has *no* unblocked
connecting path at all across either axis (can never form a road → definitely not
tinue). These are true necessary conditions, so they're safe even in Full-solve.

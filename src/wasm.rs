...
/// natural; even values round down internally). Returns a JS object with
/// shape `{ outcome: { kind, ... }, nodes }`.
#[wasm_bindgen]
pub fn solve_tinue(tps: &str, size: u8, max_plies: u32) -> JsValue {
    let response = match run(tps, size, max_plies) {
        Ok(r) => r,
...
fn run(tps: &str, size: u8, max_plies: u32) -> Result<SolveResponse, String> {
